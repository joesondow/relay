...
public class StatusWithCardJSONImpl extends TwitterResponseImpl implements StatusWithCard, java.io.Serializable {
    private static final long serialVersionUID = 4651240015141500402L;
    ...
    /* package */ StatusWithCardJSONImpl(HttpResponse res, Configuration conf) throws TwitterException {
        super(res);
    ...
    /* package */ StatusWithCardJSONImpl(JSONObject json, Configuration conf) throws TwitterException {
        super();
        init(json);
        if (conf.isJSONStoreEnabled()) {
            TwitterObjectFactory.registerJSONObject(this, json);
    ...
    /* package */ StatusWithCardJSONImpl(JSONObject json) throws TwitterException {
        super();
        init(json);
    }

    /* Only for serialization purposes. */
    /* package */ StatusWithCardJSONImpl() {
    ...
    private void init(JSONObject json) throws TwitterException {
        ...
        try {
            // We're here for poll results so let's do this.
            if (!json.isNull("card")) {
                card = new CardJSONImpl(json.getJSONObject("card"));
            }
            if (!json.isNull("user")) {
    ...
    @Override
    public int compareTo(StatusWithCard that) {
    ...
    public boolean equals(Object obj) {
        ...
        return obj instanceof StatusWithCard && ((StatusWithCard) obj).getId() == this.id;
    }

    @Override
    public String toString() {
        return "StatusWithCardJSONImpl{" + "createdAt=" + createdAt + ", id=" + id + ", text='" + text + '\''
                + ", source='" + source + '\'' + ", isTruncated=" + isTruncated + ", inReplyToStatusId="
                + inReplyToStatusId + ", inReplyToUserId=" + inReplyToUserId + ", isFavorited=" + isFavorited
                + ", isRetweeted=" + isRetweeted + ", favoriteCount=" + favoriteCount + ", inReplyToScreenName='"
                + inReplyToScreenName + '\'' + ", geoLocation=" + geoLocation + ", place=" + place + ", retweetCount="
                + retweetCount + ", isPossiblySensitive=" + isPossiblySensitive + ", lang='" + lang + '\''
                + ", contributorsIDs=" + Arrays.toString(contributorsIDs) + ", retweetedStatus=" + retweetedStatus
                + ", userMentionEntities=" + Arrays.toString(userMentionEntities) + ", urlEntities="
                + Arrays.toString(urlEntities) + ", hashtagEntities=" + Arrays.toString(hashtagEntities)
                + ", mediaEntities=" + Arrays.toString(mediaEntities) + ", symbolEntities="
                + Arrays.toString(symbolEntities) + ", currentUserRetweetId=" + currentUserRetweetId + ", user=" + user
                + ", withHeldInCountries=" + Arrays.toString(withheldInCountries) + ", quotedStatusId=" + quotedStatusId
                + ", quotedStatus=" + quotedStatus + ", card=" + card + '}';
    }
}
